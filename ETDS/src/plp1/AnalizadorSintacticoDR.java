...
public class AnalizadorSintacticoDR {
	...
	private void SInstr() {
		if(token.tipo == Token.LLAVEI || token.tipo == Token.ID || token.tipo == Token.IF || token.tipo == Token.INT || token.tipo == Token.FLOAT){
			if(flag)
				System.out.print(" 13");
			Instr();
			SInstrp();
			...
			errorSintaxis(Token.LLAVEI,Token.INT,Token.FLOAT,Token.IF,Token.ID);
		}
	}
	...
	private void SInstrp() {
		if(token.tipo == Token.LLAVEI || token.tipo == Token.ID || token.tipo == Token.IF || token.tipo == Token.INT || token.tipo == Token.FLOAT){
			...
			errorSintaxis(Token.LLAVEI,Token.LLAVED,Token.INT,Token.FLOAT,Token.IF,Token.ID);
		}
	}
